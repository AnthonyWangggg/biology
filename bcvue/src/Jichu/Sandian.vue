...
export default {
  ...
  mounted() {
    ...
    this.formInline.no = 'Scatter' + this.formInline.no
  },
  methods: {
    removeColor(item) {
    ...
    chartCommit(){
      ...
      if(_this.formInline.size!==''){
        console.log(_this.formInline.size)
        _this.pointSize = _this.formInline.size
      }
      console.log(_this.pointSize)
      ...
      for (let i = 0; i < _this.colorInline.color1.length; i++){
        if(_this.colorInline.color1[i].color!=null){
          _this.pointColor[i] = _this.colorInline.color1[i].color
        }
      }
    ...
    init_color(){
      let _this = this;
      if(_this.pointColor.length >= _this.yData.length){
